...
export const loyaltyRewards = pgTable("loyalty_rewards", {
  ...
  rewardType: text("reward_type").notNull(), // 'discount' | 'free_service' | 'product' | 'upgrade'
  rewardValue: text("reward_value"), // monto descuento o id del servicio
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const notifications = pgTable("notifications", {
  id: uuid("id").primaryKey().defaultRandom(),
  type: text("type").notNull(), // 'reminder' | 'confirmation' | 'promotion' | 'low_stock' | 'review'
  channel: text("channel").notNull(), // 'whatsapp' | 'email' | 'in_app'
  recipientId: text("recipient_id"), // phone o email
  title: text("title"),
...
export const loyaltyPointsRelations = relations(loyaltyPoints, ({ one, many }) => ({
  client: one(clients, { fields: [loyaltyPoints.clientId], references: [clients.id] }),
  tier: one(loyaltyTiers, { fields: [loyaltyPoints.tierId], references: [loyaltyTiers.id] }),
  transactions: many(loyaltyTransactions),
}));

export const loyaltyTransactionsRelations = relations(loyaltyTransactions, ({ one }) => ({
  client: one(clients, { fields: [loyaltyTransactions.clientId], references: [clients.id] }),
  appointment: one(appointments, { fields: [loyaltyTransactions.appointmentId], references: [appointments.id] }),
}));
...
export const reviewsRelations = relations(reviews, ({ one }) => ({
  ...
  colaborador: one(colaboradores, { fields: [reviews.colaboradorId], references: [colaboradores.id] }),
...
export const orders = pgTable("orders", {
  ...
  clientId: uuid("client_id")
    .references(() => clients.id, { onDelete: "cascade" })
    .notNull(),
  status: text("status").notNull().default("pendiente"), // pendiente, enviado, entregado, cancelado
  totalAmount: decimal("total_amount", { precision: 10, scale: 2 }).notNull(),
  paymentMethod: text("payment_method"), // yape, plin, efectivo, transferencia, mercadopago
  paymentStatus: text("payment_status").default("pendiente"), // pendiente, pagado, reembolsado
  paidAt: timestamp("paid_at"),
  shippingAddress: text("shipping_address"),
  deliveryMethod: text("delivery_method").default("recojo"), // recojo, delivery
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const orderItemsRelations = relations(orderItems, ({ one }) => ({
  ...
  product: one(inventory, { fields: [orderItems.inventoryId], references: [inventory.id] }),
}));

export const referralCodesRelations = relations(referralCodes, ({ one, many }) => ({
  client: one(clients, { fields: [referralCodes.clientId], references: [clients.id] }),
  usage: many(referralUsage),
}));

export const referralUsageRelations = relations(referralUsage, ({ one }) => ({
  referralCode: one(referralCodes, { fields: [referralUsage.referralCodeId], references: [referralCodes.id] }),
  referredClient: one(clients, { fields: [referralUsage.referredClientId], references: [clients.id] }),
}));
