...
const stockTools_1 = require("./tools/stockTools");
const appointmentTools_1 = require("./tools/appointmentTools");
const serviceTools_1 = require("./tools/serviceTools");
...
const toolImpl = {
    consultarStock: async (args) => (0, stockTools_1.consultarStock)(args.producto),
    consultarCitas: async (args) => (0, appointmentTools_1.consultarCitas)(args.fecha, args.colaboradora),
    consultarServicios: async () => (0, serviceTools_1.consultarServicios)(),
};
...
async function processMessage(userMessage) {
    trimHistory();
    conversationHistory.push({ role: "user", parts: [(0, genai_1.createPartFromText)(userMessage)] });
    for (let turn = 0; turn < 5; turn++) {
        const response = await ai.models.generateContent({
            model: MODEL,
            contents: conversationHistory,
            config: {
                systemInstruction: { parts: [(0, genai_1.createPartFromText)(SYSTEM_INSTRUCTION)] },
        ...
        if (calls && calls.length > 0) {
            const modelParts = calls.map((call) => (0, genai_1.createPartFromFunctionCall)(call.name, call.args ?? {}));
            ...
            for (const call of calls) {
                ...
                if (!impl) {
                    conversationHistory.push({
                        role: "function",
                        parts: [
                            (0, genai_1.createPartFromFunctionResponse)(call.name, call.name, { error: `Función "${call.name}" no encontrada.` }),
                        ],
                    });
                    continue;
                }
                try {
                    ...
                    conversationHistory.push({
                        role: "function",
                        parts: [
                            (0, genai_1.createPartFromFunctionResponse)(call.name, call.name, { result }),
                        ],
                    });
                }
                catch (err) {
                    conversationHistory.push({
                        role: "function",
                        parts: [
                            (0, genai_1.createPartFromFunctionResponse)(call.name, call.name, { error: err.message }),
                        ],
                    });
                }
            }
        ...
        if (text) {
            conversationHistory.push({ role: "model", parts: [(0, genai_1.createPartFromText)(text)] });
            return text;
        }
